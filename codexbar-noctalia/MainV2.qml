...
Item {
  ...
  function refreshUsage() {
    ...
    if (selected === "all") {
      configProcess.command = [effectiveCodexbarPath(), "config", "dump", "--format", "json", "--json-only", "--log-level", "critical"]
  ...
  function startNextUsageProvider() {
    ...
    var args = [effectiveCodexbarPath(), "usage", "--source", source, "--provider", currentUsageProvider, "--format", "json", "--json-only", "--log-level", "critical"]
  ...
  function extractedJsonDocuments(text) {
    var docs = []
    ...
    for (var i = 0; i < s.length; i++) {
      ...
      if (ch === "\"") {
        ...
        depth--
        if (depth === 0 && start >= 0) {
          docs.push(JSON.parse(s.substring(start, i + 1)))
          start = -1
        }
      }
    }
  ...
  function parseJsonDocuments(text) {
    var trimmed = stripAnsi(text).trim()
    if (!trimmed) return []

    try {
      return [JSON.parse(trimmed)]
    } catch (wholeError) {
      var docs = extractedJsonDocuments(trimmed)
